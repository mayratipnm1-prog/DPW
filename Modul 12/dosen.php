...
// --- EDIT MODE ---
$editMode = false;
$editData = ['idDosen' => '', 'namaDosen' => '', 'noHP' => ''];
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6">
            <div class="flex justify-between h-16 items-center">
                ...
                <div class="flex gap-1 bg-slate-100/80 p-1 rounded-xl">
                    <a href="dosen.php" class="px-4 py-2 rounded-lg text-sm font-medium bg-white text-blue-600 shadow-sm transition-all"><i class="fas fa-chalkboard-user mr-1.5 text-xs"></i>Dosen</a>
                    <a href="mahasiswa.php" class="px-4 py-2 rounded-lg text-sm font-medium text-slate-500 hover:text-slate-700 transition-all"><i class="fas fa-user-graduate mr-1.5 text-xs"></i>Mahasiswa</a>
                ...
                <div class="glass rounded-2xl shadow-lg shadow-slate-200/50 border border-white/50 overflow-hidden sticky top-24">
                    <div class="px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-slate-50/80 to-white/80">
                        <div class="flex items-center gap-2">
                            <div class="w-8 h-8 rounded-lg <?php echo $editMode ? 'bg-amber-100 text-amber-600' : 'bg-blue-100 text-blue-600'; ?> flex items-center justify-center">
                                <i class="fas <?php echo $editMode ? 'fa-pen' : 'fa-user-plus'; ?> text-sm"></i>
                            </div>
                            <h2 class="text-lg font-bold text-slate-800"><?php echo $editMode ? "Ubah Data Dosen" : "Tambah Dosen Baru"; ?></h2>
                        </div>
                        <p class="text-xs text-slate-500 mt-1"><?php echo $editMode ? "Perbarui informasi dosen yang sudah ada." : "Isi form berikut untuk menambahkan dosen baru."; ?></p>
                    </div>
                    
                    <form action="dosen.php" method="POST" class="p-6 space-y-5">
                        <?php if ($editMode): ?>
                        ...
                        <div>
                            ...
                            <input type="text" name="namaDosen" required value="<?php echo htmlspecialchars($editData['namaDosen']); ?>" placeholder="Masukkan nama lengkap..." class="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all text-sm">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="noHP" required value="<?php echo htmlspecialchars($editData['noHP']); ?>" placeholder="Contoh: 08123456789" class="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all text-sm">
                        </div>

                        <div class="pt-2">
                            <?php if ($editMode): ?>
                            <button type="submit" name="ubah" class="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold py-3 rounded-xl transition-all shadow-lg shadow-blue-500/25 btn-press flex items-center justify-center gap-2">
                                <i class="fas fa-save text-sm"></i> Simpan Perubahan
                            </button>
                            <a href="dosen.php" class="block text-center mt-3 text-sm text-slate-500 hover:text-slate-800 transition">Batal Edit</a>
                            <?php else: ?>
                            <button type="submit" name="tambah" class="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold py-3 rounded-xl transition-all shadow-lg shadow-blue-500/25 btn-press flex items-center justify-center gap-2">
            ...
            <div class="lg:col-span-8">
                <div class="glass rounded-2xl shadow-lg shadow-slate-200/50 border border-white/50 overflow-hidden">
                    <div class="px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-slate-50/80 to-white/80 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <form action="dosen.php" method="get" class="flex gap-2">
                            <div class="relative">
                            ...
                            <?php if ($keyword): ?>
                            <a href="dosen.php" class="px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-sm transition btn-press flex items-center"><i class="fas fa-rotate-left text-xs"></i></a>
                            <?php endif; ?>
                        </form>
                    </div>
                                ...
                                <tr class="bg-white hover:bg-blue-50/40 transition-colors group">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <div class="flex items-center justify-center gap-2 opacity-80 group-hover:opacity-100 transition-opacity">
                                            <a href="dosen.php?action=edit&id=<?php echo $row['idDosen']; ?>" class="flex items-center gap-1.5 px-3 py-2 bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 rounded-lg text-xs font-semibold transition btn-press"><i class="fas fa-pen text-[10px]"></i>Edit</a>
                                            <button onclick="confirmDelete('dosen.php?action=delete&id=<?php echo $row['idDosen']; ?>', '<?php echo htmlspecialchars(addslashes($row['namaDosen'])); ?>')" class="flex items-center gap-1.5 px-3 py-2 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 rounded-lg text-xs font-semibold transition btn-press cursor-pointer"><i class="fas fa-trash text-[10px]"></i>Hapus</button>
                                        </div>
                                    </td>
                                </tr>
                                <?php endwhile; else: ?>
