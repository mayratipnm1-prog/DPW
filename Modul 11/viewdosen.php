        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6">
            <div class="flex justify-between h-16 items-center">
                ...
                <div class="flex gap-1 bg-slate-100/80 p-1 rounded-xl">
                    <a href="viewdosen.php" class="px-4 py-2 rounded-lg text-sm font-medium bg-white text-blue-600 shadow-sm transition-all">
                        <i class="fas fa-chalkboard-user mr-1.5 text-xs"></i>Dosen
                    </a>
                    <a href="viewmahasiswa.php" class="px-4 py-2 rounded-lg text-sm font-medium text-slate-500 hover:text-slate-700 transition-all">
                ...
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-slate-100 text-sm">
                        ...
                        <tr class="bg-white hover:bg-blue-50/40 transition-colors group">
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-slate-100 text-slate-600 font-bold text-xs group-hover:bg-blue-100 group-hover:text-blue-600 transition-colors"><?php echo $data['idDosen']; ?></span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full bg-gradient-to-br from-blue-100 to-indigo-200 flex items-center justify-center text-blue-700 text-xs font-bold"><?php echo strtoupper(substr($data['namaDosen'],0,1)); ?></div>
                                    <span class="font-semibold text-slate-800"><?php echo htmlspecialchars($data['namaDosen']); ?></span>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2 opacity-80 group-hover:opacity-100 transition-opacity">
                                    <a href="editdosen.php?idDosen=<?php echo $data['idDosen']; ?>" class="flex items-center gap-1.5 px-3 py-2 bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 rounded-lg text-xs font-semibold transition btn-press"><i class="fas fa-pen text-[10px]"></i>Edit</a>
                                    <button onclick="confirmDelete('hapusdosen.php?idDosen=<?php echo $data['idDosen']; ?>', '<?php echo htmlspecialchars(addslashes($data['namaDosen'])); ?>')" class="flex items-center gap-1.5 px-3 py-2 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 rounded-lg text-xs font-semibold transition btn-press cursor-pointer"><i class="fas fa-trash text-[10px]"></i>Hapus</button>
                                </div>
                            </td>
                        </tr>
                        <?php endwhile; else: ?>
                        <tr><td colspan="4" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center text-slate-400">
                                ...
                                <p class="text-xs text-slate-400 mt-1">Klik "Tambah Dosen" untuk menambahkan data</p>
                            </div>
                        </td></tr>
                        <?php endif; ?>
                    </tbody>
